...
def read_entity_from_id(filename='./data/WN18RR/entity2id_inductive.txt'):
    entity2id = {}
...
def read_relation_from_id(filename='./data/WN18RR/relation2id_inductive.txt'):
    relation2id = {}
    with open(filename, 'r') as f:
        for line in f:
            if len(line.strip().split('\t')) > 1:
...
def build_data(path='./data/WN18RR/', is_unweigted=False, directed=True):
    entity2id = read_entity_from_id(path + 'entity2id_inductive.txt')
    relation2id = read_relation_from_id(path + 'relation2id_inductive.txt')
    ...
    train_triples, train_adjacency_mat, unique_entities_train = load_data(os.path.join(
        path, 'train_inductive.txt'), entity2id, relation2id, is_unweigted, directed)
    validation_triples, valid_adjacency_mat, unique_entities_validation = load_data(
        os.path.join(path, 'valid_inductive.txt'), entity2id, relation2id, is_unweigted, directed)
    test_triples, test_adjacency_mat, unique_entities_test = load_data(os.path.join(
        path, 'test_inductive.txt'), entity2id, relation2id, is_unweigted, directed)
 #   all_triples, all_adjacency_mat, unique_entities_all = load_data(os.path.join(
  #      path, 'all_inductive.txt'), entity2id, relation2id, is_unweigted, directed)

    id2entity = {v: k for k, v in entity2id.items()}
    id2relation = {v: k for k, v in relation2id.items()}
    left_entity, right_entity = {}, {}

    with open(os.path.join(path, 'train_inductive.txt')) as f:
        lines = f.readlines()
    ...
    return (train_triples, train_adjacency_mat), (validation_triples, valid_adjacency_mat), (test_triples, test_adjacency_mat), \
        entity2id, relation2id, headTailSelector, unique_entities_train, unique_entities_validation, unique_entities_test

...
class SubgraphDataset(Dataset):
    ...
    def __getitem__(self, index):
        with self.main_env.begin(db=self.db_pos) as txn:
         ...
         #   print('nodes_pos shape ', len(nodes_pos))
         #   print('nodes_pos ',nodes_pos)   #############nodes
          #  print('r_label_pos shape ',r_label_pos.shape)   #####relation target  shape 1
          #  print('r_label_pos ',r_label_pos)    
          #  print('g_label_pos shape ',g_label_pos.shape)  ###########graph label 0 or 1 shape 1
          #  print('g_label_pos ',g_label_pos) 
            subgraph_pos = self._prepare_subgraphs(nodes_pos, r_label_pos, n_labels_pos)
           # print('subgraph_pos ', len(subgraph_pos))
        subgraphs_neg = []
        r_labels_neg = []
        g_labels_neg = []
        with self.main_env.begin(db=self.db_neg) as txn:
    ...
    def _prepare_subgraphs(self, nodes, r_label, n_labels):
        ...
        if rel_link.squeeze().nelement() == 0:
           ...
           # print('all edges length ', len(subgraph.edges()[0]))  #########source: subgraph.edges()[0] target: subgraph.edges()[1]
    ...
    def _prepare_features_new(self, subgraph, n_labels, n_feats=None):
      ...
      #  subgraph.ndata['head_id'] = head_id
      #  subgraph.ndata['tail_id'] = tail_id        
        self.n_feat_dim = n_feats.shape[1]  # Find cleaner way to do this -- i.e. set the n_feat_dim
        return subgraph
